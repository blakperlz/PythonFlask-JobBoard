--- original: C:\Users\jeffw\Documents\Repo\PluralSight\FlaskDemo\PythonFlask-JobBoard\jobs\app.py
+++ fixed: C:\Users\jeffw\Documents\Repo\PluralSight\FlaskDemo\PythonFlask-JobBoard\jobs\app.py
@@ -4,6 +4,7 @@
 PATH = 'db/jobs.sqlite'
 
 app = Flask(__name__)
+
 
 def open_connection():
 	connection = getattr(g, '_connection', None)
@@ -12,10 +13,11 @@
 	connection.row_factory = sqlite3.Row
 	return connection
 
+
 def execute_sql(sql, values=(), commit=False, single=False):
-	connection=open_connection()
-	cursor=connection.execute(sql, values)
-	if commit==True:
+	connection = open_connection()
+	cursor = connection.execute(sql, values)
+	if commit == True:
 		results = connection.commit()
 	else:
 		results = cursor.fetchone() if single else cursor.fetchall()
@@ -23,9 +25,10 @@
 	cursor.close()
 	return results
 
+
 @app.teardown_appcontext
 def close_connection(exception):
-	connection=getattr(g, '_connection', None)
+	connection = getattr(g, '_connection', None)
 	if connection is not None:
 		connection.close()
@@ -33,20 +36,23 @@
 @app.route('/')
 @app.route('/jobs')
 def jobs():
-	jobs=execute_sql('SELECT job.id, job.title, job.description, job.salary, employer.id as employer_id, employer.name as employer_name FROM job JOIN employer ON employer.id = job.employer_id')
+	jobs = execute_sql('SELECT job.id, job.title, job.description, job.salary, employer.id as employer_id, employer.name as employer_name FROM job JOIN employer ON employer.id = job.employer_id')
 	return render_template('index.html', jobs=jobs)
+
 @app.route('/job/<job_id>')
 def job(job_id):
-	job = execute_sql('SELECT job.id, job.title, job.description, job.salary, employer.id as employer_id, employer.name as employer_name FROM job JOIN employer ON employer.id = job.employer_id WHERE job.id = ?', [job_id], single=True)
-	#job = execute_sql('SELECT job.id, job.title, job.single=True)		
+	job = execute_sql('SELECT job.id, job.title, job.description, job.salary, employer.id as employer_id, employer.name as employer_name FROM job JOIN employer ON employer.id = job.employer_id WHERE job.id = ?', [
+	                  job_id], single=True)
+	# job = execute_sql('SELECT job.id, job.title, job.single=True)
 	return render_template('job.html', job=job)
+
 
 @app.route('/employer/<employer_id>')
 def employer(employer_id):
 	employer = execute_sql('SELECT * FROM employer WHERE id=?',
-	[employer_id]), singe=True)
-	jobs = execute_sql('SELECT job.id, job.title, job.description,[employer_id]), singe = True)
+	jobs=execute_sql('SELECT job.id, job.title, job.description,
 	job.salary FROM job JOIN employer on employer.id = job.employer_id WHERE
 	employer.id = ?')
-	return render_template('employer.html', employer=employer)
+	return render_template('employer.html', employer = employer)
